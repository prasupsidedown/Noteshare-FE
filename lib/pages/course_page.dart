...
class _CoursePageState extends State<CoursePage> {
  ...
  // FAB drag position — default tengah bawah
  double _fabX = -1; // -1 berarti belum diinisialisasi
  double _fabY = -1;
  ...
  void _initFabPosition(BoxConstraints constraints) {
    if (!_fabInitialized) {
      _fabInitialized = true;
      // Default: tengah bawah saat belum ada matkul
      _fabX = constraints.maxWidth / 2 - 28;
      _fabY = constraints.maxHeight - 80;
    }
  }

  void _snapFabToIdealPosition(BoxConstraints constraints) {
    // Setelah ada matkul, pindah ke kanan bawah
    setState(() {
    ...
    return Scaffold(
      body: LayoutBuilder(
        builder: (context, constraints) {
          // Inisialisasi posisi FAB pertama kali
          if (!_fabInitialized) {
            WidgetsBinding.instance.addPostFrameCallback((_) {
              if (mounted) {
                setState(() {
                  _fabInitialized = true;
                  if (_courses.isEmpty) {
                    // Tengah bawah saat kosong
                    _fabX = constraints.maxWidth / 2 - 28;
                    _fabY = constraints.maxHeight - 80;
                  } else {
                    // Kanan bawah saat ada matkul
                    _fabX = constraints.maxWidth - 72;
                    _fabY = constraints.maxHeight - 80;
                  }
                });
              ...
              if (_fabInitialized)
                Positioned(
                  ...
                  child: GestureDetector(
                    ...
                    onPanEnd: (_) {
                      // Snap ke tepi terdekat setelah dilepas
                      final centerX = constraints.maxWidth / 2;
                      setState(() {
                        if (_fabX + 28 < centerX) {
                          _fabX = 16; // snap kiri
                        } else {
                          _fabX = constraints.maxWidth - 72; // snap kanan
                        }
                        // Clamp vertical
